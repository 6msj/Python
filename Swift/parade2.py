...
def detect_errors(listRequirements, holdNodes):
    ...
    for requirement in listRequirements[1:len(listRequirements)]:
        ...
        if firstNode:
            ...
            create_second_node(firstNode, secondNode, holdNodes, requirement)
        else:
...
def create_second_node(firstNode, secondNode, holdNodes, requirement):
    ...
    if secondNode:
        ...
        # Second name wasn't found either. Create it and proceed.
...
def check_if_any_node_brings_an_error(firstNode, secondNode):
...
def add_names_to_set(firstNode, secondNode, holdNodes):
    ...
    if check_if_any_node_brings_an_error(firstNode, secondNode):
        return False
    else:
        return True
...
#def append_final_nodes(firstNode, holdNodes):
    #for node in holdNodes:
        #print node.name
    #print " _________ "
    ## If nothing is after this node.
    #listNodes = []
    #if not firstNode.after:
        #return [firstNode]
    #else:
        #for node in firstNode.after:
            #holdNodes.remove(node)
            #listNodes = listNodes + [firstNode] + append_final_nodes(node, holdNodes)
        ...
        #return sorted_list(listNodes)

#def sorted_list(listNodes):
    #print "*****"
    ...
    #for firstList in listNodes:
        #for nextList in listNodes[1:len(listNodes)]:
            #for oneNode in firstList:
                #if oneNode in nextList:
                    #print oneNode.name
                    ## take care of what happens 
            #firstList = firstList + nextList
            #listNodes.remove(nextList)
...
def main():
    ...
    firstNode = find_firstNode(holdNodes)
    #output += [firstNode]
    #output + append_final_nodes(firstNode, holdNodes)
    for node in output:
        print node.name
